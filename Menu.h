...
public:
	...
	/*!parameter!*/ bool NewDesign = false; //Turn on this parameter if you want to use new design!
	...
	{
		txSetColor((button->status == BS_MOUSEOVER) ? TX_WHITE : ((NewDesign) ? RGB(180, 0, 255) : button->color));
		txSetFillColor((button->status == BS_MOUSEOVER) ? TX_WHITE : ((NewDesign) ? RGB(180, 0, 255) : button->color));

		int hint = (button->status == BS_MOUSEOVER) ? 0 : 0;

		txRectangle(button->x1 - hint, button->y1 - hint, button->x2 + hint, button->y2 + hint);

		txSetFillColor((button->status == BS_MOUSEOVER) ? TX_WHITE : ((NewDesign) ?  button->color : RGB(180, 0, 255)));
		txSetColor((button->status == BS_MOUSEOVER) ? TX_WHITE : ((NewDesign) ?  button->color : RGB(180, 0, 255)));

		if (NewDesign) txRectangle(button->x1 + (button->x2 - button->x1) / 40 * 39, button->y1, button->x2, button->y2);
